...
import { z } from "zod"
import type { DailyFoodSupplierConfig } from "./types.js"

const DailyFoodConfigFileSchema = z.object({
  supplier_id: z.literal("dailyfood"),
...
export class DailyFoodConfigError extends Error {
  readonly name = "DailyFoodConfigError"

  constructor(
    readonly configPath: string,
    options?: ErrorOptions,
  ) {
    super(`Invalid DailyFood supplier config: ${configPath}`, options)
  }
}

export async function loadDailyFoodSupplierConfig(
  configPath: string,
): Promise<DailyFoodSupplierConfig> {
  try {
    const value = DailyFoodConfigFileSchema.parse(parse(await readFile(configPath, "utf8")))
    return {
    ...
    throw new DailyFoodConfigError(configPath, { cause: error })
  }
}
