...
const ACTIONS = ["add_variation_candidate", "create_draft_product_candidate"] as const
...
type Decision = "safe" | "already_exists" | "duplicate_hold" | "review_needed" | "blocked"
...
export async function executeMvpAddCreate(options: {
...
}): Promise<{ readonly report: ExecuteReport; readonly verification: Verification }> {
  if (!options.execute || options.confirm !== "EXECUTE_MVP_ADD_VARIATIONS_AND_CREATE_DRAFTS")
    throw new Error(
      '--execute --confirm "EXECUTE_MVP_ADD_VARIATIONS_AND_CREATE_DRAFTS" is required',
    )
  const rows = PlanSchema.parse(JSON.parse(await readFile(options.planPath, "utf8"))).rows.filter(
    (row) => ACTIONS.includes(row.action as (typeof ACTIONS)[number]),
  )
  ...
  for (const group of groupCreateRows(
    safety.filter((item) => item.decision === "safe" && item.mode === "create_draft_product"),
  ))
...
function buildSafety(
  ...
  const existingNames = new Set(catalog.map((product) => clean(product.name)))
  ...
  for (const row of rows.filter((item) => item.action === "create_draft_product_candidate"))
    createGroups.set(clean(row.woocommerce_product_name), [
      ...(createGroups.get(clean(row.woocommerce_product_name)) ?? []),
      row,
    ])
  for (const [key, group] of createGroups) {
    const reasons = reviewCreateGroup(key, group, existingNames)
...
function reviewAdd(
  ...
  if (matchExcludedProduct(`${row.woocommerce_product_name} ${row.woocommerce_option_name}`))
  ...
  if (duplicateKeys.get(`${row.product_id}:${optionKey(row.woocommerce_option_name)}`) !== 1)
    reasons.push("duplicate_candidate_option")
  if (
    product?.variations.some(
      (variation) => optionKey(optionName(variation)) === optionKey(row.woocommerce_option_name),
    )
  )
...
function reviewCreateGroup(
  ...
  if (existingNames.has(key)) reasons.push("duplicate_existing_or_draft_product")
  if (!validName(first.woocommerce_product_name)) reasons.push("unclear_product_name")
  if (
    matchExcludedProduct(
      rows.map((row) => `${row.woocommerce_product_name} ${row.woocommerce_option_name}`).join(" "),
    )
  )
    reasons.push("livestock_excluded")
...
async function addVariation(
  ...
  try {
    ...
    const variation = VariationSchema.parse(
      await ky
        .post(`${client.baseUrl}/wp-json/wc/v3/products/${row.product_id}/variations`, {
      ...
      await ky
        .post(`${client.baseUrl}/wp-json/wc/v3/products`, {
          ...
          json: {
            ...
            meta_data: [{ key: "_wholesalehub_mvp_created", value: "draft_candidate" }],
          },
          timeout: 60_000,
          retry: { limit: 0 },
        })
...
function variationPayload(attributeName: string, row: SafetyRow): Record<string, unknown> {
  return {
...
function buildReport(
  ...
  return {
    ...
    addVariationCandidateCount: rows.filter((row) => row.action === "add_variation_candidate")
      .length,
    createDraftCandidateCount: rows.filter((row) => row.action === "create_draft_product_candidate")
      .length,
    ...
    safeCreateDraftProductCount: new Set(
      safety
        .filter((row) => row.mode === "create_draft_product" && row.decision === "safe")
        .map((row) => clean(row.product_name)),
    ).size,
    executedAddVariationCount: created.filter((entry) => entry.mode === "add_variation").length,
    ...
    alreadyExistsOrDuplicateHoldCount: safety.filter(
      (row) => row.decision === "already_exists" || row.decision === "duplicate_hold",
    ).length,
...
function buildVerification(
  ...
  const beforeVariationCount = before.reduce((sum, product) => sum + product.variations.length, 0)
  const afterVariationCount = after.reduce((sum, product) => sum + product.variations.length, 0)
  return {
    ...
    afterVariationCount,
    beforeDraftCount: before.filter((product) => product.status === "draft").length,
    afterDraftCount: after.filter((product) => product.status === "draft").length,
...
async function writeReports(
  ...
  await Promise.all([
    writeFile(resolve(dir, "mvp-add-create-safety-review.csv"), safetyCsv(safety), "utf8"),
    writeFile(
...
function groupCreateRows(rows: readonly SafetyRow[]): readonly (readonly SafetyRow[])[] {
  const groups = new Map<string, SafetyRow[]>()
  for (const row of rows)
    groups.set(clean(row.product_name), [...(groups.get(clean(row.product_name)) ?? []), row])
  return [...groups.values()]
}
function countKeys<T>(rows: readonly T[], keyOf: (row: T) => string): ReadonlyMap<string, number> {
...
function optionKey(value: string): string {
  const matches = [
    ...value.matchAll(/\d+(?:\.\d+)?\s*(?:kg|g|개입|개|팩|봉|박스|망|과|R|센치|cm)/giu),
