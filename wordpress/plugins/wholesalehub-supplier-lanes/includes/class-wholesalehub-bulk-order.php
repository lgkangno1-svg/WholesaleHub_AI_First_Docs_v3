...
final class WholesaleHub_Bulk_Order {
    ...
    public static function account_menu(array $items): array { $logout=$items['customer-logout']??null;unset($items['customer-logout']);$items['bulk-order']='엑셀 대량주문';if($logout!==null)$items['customer-logout']=$logout;return $items; }
    ...
    public static function page(): void {
        if (!self::allowed()) { echo '<p>승인된 사업자 회원만 이용할 수 있습니다.</p>'; return; }
        $batch = self::batch(absint($_GET['batch'] ?? 0), get_current_user_id());
        echo '<section class="wh-bulk-order"><h2>엑셀 대량주문</h2><p><a class="button" href="' . esc_url(admin_url('admin-post.php?action=wh_bulk_template')) . '">양식 다운로드</a></p>';
        echo '<form method="post" enctype="multipart/form-data" action="' . esc_url(admin_url('admin-post.php')) . '"><input type="hidden" name="action" value="wh_bulk_upload">'; wp_nonce_field(self::NONCE); echo '<input required type="file" name="bulk_file" accept=".xlsx,.csv"><label><input type="checkbox" name="new_batch" value="1"> 완료된 동일 파일로 새 주문 만들기</label><button class="button alt">업로드 및 검증</button></form>';
    ...
    public static function template(): void {
        ...
        $rows = [["고객주문번호","상품코드","옵션코드","수량","수령인","수령인연락처","우편번호","기본주소","상세주소","배송메시지"], ["ORDER-001","상품코드는 상품목록 시트를 확인","public_offer_key",1,"홍길동","01012345678","12345","서울특별시","101호",""]];
        self::xlsx('wholesalehub-bulk-order-template.xlsx', ['주문입력' => $rows, '상품목록' => self::catalog(), '작성안내' => [["한 행은 상품/옵션/수령지 1건입니다"],["가격과 배송비는 서버에서 다시 계산됩니다"],["같은 고객주문번호와 주소는 한 배송건으로 묶입니다"]]]);
    }
    ...
    public static function upload(): void {
        ...
        if (!$rows || count($rows) > self::MAX_ROWS + 1) self::redirect_error('빈 파일이거나 최대 1,000행을 초과했습니다.');
    ...
    private static function xlsx_rows(string $file):array{if(!class_exists('ZipArchive'))return []; $z=new ZipArchive();if($z->open($file)!==true)return [];$compressed=0;$expanded=0;if($z->numFiles<1||$z->numFiles>100){$z->close();return [];}for($i=0;$i<$z->numFiles;$i++){$s=$z->statIndex($i);$n=(string)$s['name'];$compressed+=(int)$s['comp_size'];$expanded+=(int)$s['size'];if(str_starts_with($n,'xl/externalLinks/')||$n==='xl/vbaProject.bin'){$z->close();return [];}}if($expanded>self::MAX_BYTES*4||$compressed<1||$expanded/$compressed>100){$z->close();return [];} $shared=[];$xml=$z->getFromName('xl/sharedStrings.xml');if($xml){$x=simplexml_load_string($xml);if(!$x){$z->close();return [];}foreach($x->si as $v)$shared[]=self::cell((string)$v->t);}$sheet=$z->getFromName('xl/worksheets/sheet1.xml');$z->close();if(!$sheet||str_contains($sheet,'<f'))return [];$x=simplexml_load_string($sheet);if(!$x)return [];$x->registerXPathNamespace('x','http://schemas.openxmlformats.org/spreadsheetml/2006/main');$out=[];foreach($x->xpath('//x:row') as $row){if(count($out)>self::MAX_ROWS||count($row->c)>10)return [];$line=[];foreach($row->c as $c){$v=(string)$c->v;if((string)$c['t']==='s')$v=$shared[(int)$v]??'';elseif((string)$c['t']==='inlineStr')$v=(string)$c->is->t;$line[]=self::cell($v);} $out[]=$line;}return $out;}
    private static function cell(string $value):string{$value=wp_strip_all_tags($value);return mb_strlen($value)>1000?'':trim($value);}
    private static function catalog():array{global $wpdb;$o=$wpdb->prefix.'supplier_lane_offers';$rows=[['상품코드','옵션코드','상품명','옵션명','판매가(참고)','재고상태','배송비유형','기본배송비(참고)','제주추가배송비(참고)','도서산간안내']];foreach($wpdb->get_results("SELECT public_offer_key,public_option_label,woo_parent_id,woo_variation_id,sale_price,stock_status,shipping_policy_json FROM {$o} WHERE approval_status='approved' AND lifecycle_status='active'",ARRAY_A) as $r){$p=json_decode((string)$r['shipping_policy_json'],true);$rows[]=[(string)$r['woo_parent_id'],(string)$r['public_offer_key'],get_the_title((int)$r['woo_parent_id']),(string)$r['public_option_label'],(string)$r['sale_price'],(string)$r['stock_status'],(string)($p['shipping_policy_type']??''),(string)($p['shipping_base_fee']??0),(string)($p['shipping_jeju_extra_fee']??0),(float)($p['shipping_remote_extra_fee']??0)>0?'도서산간 추가배송비 있음':'정책 없음'];}return $rows;}
    private static function xlsx(string $name,array $sheets):void{nocache_headers();header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');header('Content-Disposition: attachment; filename='.$name);$z=new ZipArchive();$tmp=tempnam(sys_get_temp_dir(),'whx');$z->open($tmp,ZipArchive::CREATE);$overrides='';for($n=1;$n<=count($sheets);$n++)$overrides.='<Override PartName="/xl/worksheets/sheet'.$n.'.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>';$z->addFromString('[Content_Types].xml','<?xml version="1.0"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/>'.$overrides.'</Types>');$z->addFromString('_rels/.rels','<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>');$wb='<?xml version="1.0"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>'; $rels='<?xml version="1.0"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">';$i=1;foreach($sheets as $title=>$rows){$wb.='<sheet name="'.esc_attr($title).'" sheetId="'.$i.'" r:id="rId'.$i.'"/>'; $rels.='<Relationship Id="rId'.$i.'" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet'.$i.'.xml"/>'; $xml='<?xml version="1.0"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>';foreach($rows as $n=>$row){$xml.='<row r="'.($n+1).'">';foreach(array_values($row) as $v)$xml.='<c t="inlineStr"><is><t>'.htmlspecialchars((string)$v,ENT_XML1|ENT_QUOTES,'UTF-8').'</t></is></c>';$xml.='</row>';}$z->addFromString('xl/worksheets/sheet'.$i.'.xml',$xml.'</sheetData></worksheet>');$i++;}$z->addFromString('xl/workbook.xml',$wb.'</sheets></workbook>');$z->addFromString('xl/_rels/workbook.xml.rels',$rels.'</Relationships>');$z->close();readfile($tmp);unlink($tmp);exit;}
    public static function admin_menu():void{add_submenu_page('woocommerce','대량주문','대량주문','manage_woocommerce','wh-bulk-order',function(){global $wpdb;$t=$wpdb->prefix.'wholesalehub_bulk_batches';$rows=$wpdb->get_results("SELECT * FROM {$t} ORDER BY id DESC LIMIT 100",ARRAY_A);echo '<div class="wrap"><h1>대량주문</h1><table class="widefat"><tr><th>Batch</th><th>고객</th><th>행</th><th>배송</th><th>금액</th><th>Woo order</th><th>상태</th></tr>';foreach($rows as $r)echo '<tr><td>'.esc_html($r['uuid']).'</td><td>'.(int)$r['customer_id'].'</td><td>'.(int)$r['row_count'].'</td><td>'.(int)$r['shipment_count'].'</td><td>'.esc_html($r['grand_total']).'</td><td>'.(int)$r['woo_order_id'].'</td><td>'.esc_html($r['status']).'</td></tr>';echo '</table></div>';});}
