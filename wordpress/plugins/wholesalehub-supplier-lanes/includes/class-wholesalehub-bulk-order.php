...
final class WholesaleHub_Bulk_Order {
    ...
    public static function page(): void {
        ...
        echo '<article class="wh-bulk-step"><span>STEP 2</span><h3>엑셀 작성</h3><p>상품목록 시트의 상품코드와 옵션코드를 주문입력 시트에 입력합니다.</p></article>';
        ...
        echo '<details class="wh-bulk-help"><summary>사용방법</summary><ol><li>엑셀 양식을 다운로드합니다.</li><li>상품목록 시트에서 원하는 상품코드와 옵션코드를 확인합니다.</li><li>주문입력 시트에 상품, 수량, 수령지 정보를 입력합니다.</li><li>작성한 파일을 업로드합니다.</li><li>오류가 없으면 상품금액과 배송비를 확인합니다.</li><li>주소별 주문내역을 확인한 뒤 결제합니다.</li></ol><h3>주의사항</h3><ul><li>엑셀의 가격과 배송비는 서버에서 다시 계산됩니다.</li><li>품절되거나 판매가 종료된 상품은 주문할 수 없습니다.</li><li>오류가 있는 행은 결제할 수 없습니다.</li><li>한 번에 최대 1,000행까지 가능합니다.</li></ul></details>';
    ...
    public static function template(): void {
        ...
        $guide = [["1. 주문입력 시트에서 한 행에 하나의 상품을 입력합니다."],["2. 상품코드와 옵션코드는 상품목록 시트에서 확인합니다."],["3. 고객주문번호가 같으면 동일한 배송 건으로 처리됩니다."],["4. 별도로 발송하려면 고객주문번호를 다르게 입력합니다."],["5. 수량은 1 이상의 숫자로 입력합니다."],["6. 한 파일에 최대 1,000행까지 주문할 수 있습니다."],["7. 1,001행 이상은 파일을 나누어 업로드해야 합니다."],["8. 엑셀에 표시된 가격과 배송비는 참고용입니다."],["9. 실제 금액은 업로드 시점의 서버 가격과 배송비로 다시 계산됩니다."],["10. 제주 및 지역 추가배송비는 주소 확인 후 계산됩니다."]];
        self::xlsx('도매허브_대량주문_양식_'.wp_date('Ymd').'.xlsx', ['주문입력' => $rows, '상품목록' => self::catalog(), '작성안내' => $guide]);
    }
    ...
    private static function catalog():array{global $wpdb;$o=$wpdb->prefix.'supplier_lane_offers';$p=$wpdb->prefix.'supplier_lane_parent_links';$posts=$wpdb->posts;$rows=[['상품코드','옵션코드','상품명','옵션명','판매가','재고상태','배송비유형','기본배송비','제주추가배송비','도서산간안내']];foreach($wpdb->get_results("SELECT o.public_offer_key,o.public_option_label,o.woo_parent_id,o.woo_variation_id,o.stock_status,o.shipping_policy_json FROM {$o} o JOIN {$p} l ON l.id=o.parent_link_id AND l.status='approved' JOIN {$posts} parent ON parent.ID=o.woo_parent_id AND parent.post_status='publish' JOIN {$posts} variation ON variation.ID=o.woo_variation_id AND variation.post_status='publish' WHERE o.approval_status='approved' AND o.lifecycle_status='active'",ARRAY_A) as $r){$v=wc_get_product((int)$r['woo_variation_id']);if(!$v||!$v->is_purchasable()||!$v->is_in_stock())continue;$policy=json_decode((string)$r['shipping_policy_json'],true);$rows[]=[(string)$r['woo_parent_id'],(string)$r['public_offer_key'],get_the_title((int)$r['woo_parent_id']),(string)$r['public_option_label'],(string)$v->get_price(),(string)$r['stock_status'],(string)($policy['shipping_policy_type']??''),(string)($policy['shipping_base_fee']??0),(string)($policy['shipping_jeju_extra_fee']??0),(float)($policy['shipping_remote_extra_fee']??0)>0?'도서산간 추가배송비 있음':'정책 없음'];}return $rows;}
