<?php
/**
 * WholesaleHub AI Merchandising module.
 * Queues non-blocking Codex merchandising for newly published products and
 * preserves successful AI content across later supplier catalog updates.
 */
...
function wh_ai_merchandising_merge_detail_block(string $base_content, string $detail_html): string
{
    ...
    if ($existing !== '') $base_content = str_replace($existing, '', $base_content);
...
{
    if ($product_id <= 0 || get_post_meta($product_id, '_wh_ai_thumbnail_status', true) !== 'success') return false;
...
function wh_ai_merchandising_public_facts(WC_Product $product): array
{
    ...
    if (mb_strlen($description) > 3500) $description = mb_substr($description, 0, 3500);
...
function wh_ai_merchandising_enqueue_product(int $product_id): bool
{
    ...
    $queue = trailingslashit($uploads['basedir']) . 'wholesalehub/ai-merchandising/queue';
    if (!wp_mkdir_p($queue)) {
        update_post_meta($product_id, '_wh_ai_merchandising_queue_status', 'failed');
        return false;
    }
...
{
    if ($post->post_type !== 'product' || $new_status !== 'publish' || !in_array($old_status, ['draft', 'pending'], true)) return;
...
{
    if (!empty($GLOBALS['wh_ai_merchandising_applying'])) return $data;
    $post_id = isset($postarr['ID']) ? (int) $postarr['ID'] : 0;
    if ($post_id <= 0 || get_post_type($post_id) !== 'product') return $data;
