...
function avocadoss_multi_variation_cart_ui() {
    ...
    echo '<button type="button" class="button alt avocadoss-add-selected" disabled>선택 옵션 장바구니 담기</button>';
...
function avocadoss_add_multi_variations() {
    ...
    wp_send_json_success( array( 'message' => '선택한 옵션이 장바구니에 담겼습니다.', 'cart_url' => wc_get_cart_url(), 'errors' => $errors ) );
}
...
function avocadoss_multi_variation_assets() {
    ...
    jQuery(function($){
      ...
      function refresh(box){var total=0;rows(box).each(function(){total += Number($(this).data('price')) * parseInt($(this).find('input').val()||'0',10);});box.find('.avocadoss-selected-total strong').text(money(total));box.find('.avocadoss-empty-selection').toggle(rows(box).length===0);box.find('.avocadoss-add-selected').prop('disabled',rows(box).length===0);}
      ...
      $(document).on('click','.avocadoss-add-selected',function(){var btn=$(this);var box=btn.closest('.avocadoss-multi-cart');var items=[];rows(box).each(function(){items.push({variation_id:$(this).data('variation-id'),quantity:parseInt($(this).find('input').val()||'1',10),label:$(this).data('label')});});var msg=box.find('.avocadoss-multi-cart-message');if(!items.length){msg.text('선택한 옵션이 없습니다.');return;}btn.prop('disabled',true);$.post(wc_add_to_cart_params.ajax_url,{action:'avocadoss_add_multi_variations',nonce:box.data('nonce'),product_id:box.data('product-id'),items:JSON.stringify(items)}).done(function(res){msg.text(res.data.message);$(document.body).trigger('wc_fragment_refresh');}).fail(function(xhr){var data=xhr.responseJSON&&xhr.responseJSON.data;msg.text((data&&data.message)||'장바구니 담기에 실패했습니다.');}).always(function(){btn.prop('disabled',false);refresh(box);});});
