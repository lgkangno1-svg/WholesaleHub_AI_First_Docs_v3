...
it("keeps bulk data server-side and uses the WooCommerce checkout basket", () => {
  ...
  expect(bulk).toContain("$cells=$row->children($main)->c")
  expect(bulk).toContain("['상품명','옵션명','상품페이지','판매가'")
  ...
  expect(bulk).toContain("offer_by_name")
  expect(bulk).toContain("count($matches)===1")
  expect(bulk).toContain("상품명 또는 옵션명을 찾을 수 없습니다")
  expect(bulk).toContain('Normalizer::normalize')
  expect(bulk).toContain('"고객주문번호","상품명","옵션명","수량"')
  expect(bulk).toContain("relationships/hyperlink")
  expect(bulk).toContain("'link'=>get_permalink")
  expect(bulk).not.toContain("['주문코드','상품명','옵션명','판매가'")
