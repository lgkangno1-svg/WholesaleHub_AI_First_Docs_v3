...
- src/reports/parsing-rule-auto-answer-cli.ts
- src/reports/parsing-rule-apply.ts
...
- update_variation_price 후보 수: 26
- no_op 수: 160
- sync review_needed 수: 237
...
- npm run parsing-rules:auto-answer -- --questions reports/parsing-rule-questions.csv
...
- reports/parsing-rule-questions-needs-user.csv
- reports/parsing-rule-questions-needs-user.json
...
- GET 검증 162건 / 불일치 0건.
- 이번 자동 룰 적용 작업에서는 WooCommerce 변경 없음.
...
- reports/parsing-rule-questions-needs-user.csv가 비어 있음. 새 질문이 생기면 사용자 답변 반영.
- sync review_needed 237건은 대부분 신규 생성 후보이므로 실제 생성 승인 전까지 dry-run 유지.
- 신규 상품/variation 생성은 별도 승인 전까지 금지.
